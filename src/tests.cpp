...
#define REQUIRE_THROWS_FS_ERROR(expr, errcode)                                                     \
    do                                                                                             \
    {                                                                                              \
        try                                                                                        \
        {                                                                                          \
            expr;                                                                                  \
            FAIL("Did not throw: " #expr);                                                         \
        }                                                                                          \
        catch (fs::filesystem_error & e)                                                           \
        {                                                                                          \
            REQUIRE(e.code() == std::errc::errcode);                                               \
        }                                                                                          \
    } while (0)
...
TEST_CASE("Filesystem", "[filesystem]")
{
    ...
    SECTION("Directory Iterators")
    {
        ...
        SECTION("Skips . and ..")
        {
            ...
            SECTION("directory_iterator")
            {
                ...
                REQUIRE(fs::recursive_directory_iterator{p, ec} ==
    ...
    SECTION("Operations")
    {
        SECTION("create_directories")
        {
            ...
            REQUIRE_THROWS_FS_ERROR(fs::create_directories(fs::path{"/dev/null/dir"}),
                                    not_a_directory);
        ...
        SECTION("create_directory")
        {
            ...
            REQUIRE_THROWS_FS_ERROR(fs::create_directory(fs::path{"/dev/null/dir"}),
                                    not_a_directory);
    ...
    SECTION("Path")
    {
        ...
        SECTION("replace_filename")
        {
            ...
            REQUIRE(fs::path{"file.ext"}.replace_filename(fs::path{"other.file"}).native() ==
                    "other.file");
            REQUIRE(fs::path{"/foo/bar.txt"}.replace_filename(fs::path{"baz.mid"}).native() ==
                    "/foo/baz.mid");
            REQUIRE(fs::path{"/foo/bar.txt"}.replace_filename(fs::path{"baz/qux.mid"}).native() ==
                    "/foo/baz/qux.mid");
            REQUIRE(fs::path{"/foo/bar.txt"}.replace_filename(fs::path{"/baz/qux.mid"}).native() ==
                    "/baz/qux.mid");
        ...
        SECTION("replace_extension")
        {
            ...
            REQUIRE(fs::path{"file.ext"}.replace_extension(fs::path{"newext"}).native() ==
                    "file.newext");
            REQUIRE(fs::path{"file.ext"}.replace_extension(fs::path{".newext"}).native() ==
                    "file.newext");
            REQUIRE(fs::path{"file.ext"}.replace_extension(fs::path{"..newext"}).native() ==
